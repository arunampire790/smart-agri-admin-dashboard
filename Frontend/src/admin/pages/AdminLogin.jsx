    ...
    <div className="min-h-screen bg-surface flex relative overflow-hidden">
      ...
      <div className="hidden lg:flex flex-col justify-center items-center w-1/2 p-12 relative">
        <div className="max-w-md text-center">
          <div className="w-16 h-16 bg-brand rounded-2xl flex items-center justify-center text-white mx-auto mb-6" style={{ boxShadow: '0 8px 30px rgba(16,185,129,0.3)' }}>
            <i className="ph ph-plant text-3xl" />
          </div>
          <div className="text-3xl font-bold text-[#111827] mb-3">Smart Agriculture</div>
          <div className="text-sm text-[#4B5563] mb-10 leading-relaxed">Intelligent farm management platform — monitor fields, control robots, and optimize yields from a single dashboard.</div>

          <div className="flex flex-col gap-4">
            {[
              { icon: 'ph-sun', label: 'Real-Time Sensor Data', sub: 'Live soil nutrition and camera feeds' },
              { icon: 'ph-robot', label: 'Robot Fleet Control', sub: 'Monitor and manage your AgriBots' },
              { icon: 'ph-chart-line', label: 'Analytics & Reports', sub: 'Crop yield and productivity insights' },
            ].map((feat) => (
              <div key={feat.label} className="flex items-center gap-4 p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.4)' }}>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'rgba(16,185,129,0.12)' }}>
                  <i className={`${feat.icon} text-lg`} style={{ color: '#059669' }} />
                </div>
                <div className="text-left">
                  <div className="text-sm font-semibold text-[#1C1C1E]">{feat.label}</div>
                  <div className="text-xs text-text-secondary">{feat.sub}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Right panel — login card / forgot password flow */}
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-[400px]">
          <div className="rounded-2xl p-8" style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', border: '1px solid rgba(255,255,255,0.6)', boxShadow: '0 20px 40px rgba(0,0,0,0.06), 0 1px 3px rgba(0,0,0,0.02)' }}>
            {/* Logo row - only show on mobile since desktop has it on left */}
            ...
            <div className={`text-center ${step === 'login' ? 'mb-6' : 'mb-6'}`}>
              ...
              <div className="text-2xl font-bold" style={{ color: '#111827' }}>
                {step === 'login' ? 'Welcome Back' : 'Smart Agriculture'}
              </div>
              <div className="text-sm mt-0.5" style={{ color: '#4B5563' }}>
                {step === 'login' && 'Sign in to your admin panel'}
