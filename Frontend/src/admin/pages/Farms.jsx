...
export default function Farms() {
  ...
  const [formBoundaryMeta, setFormBoundaryMeta] = useState({ polygonClosed: false });
  const [editBoundaryMeta, setEditBoundaryMeta] = useState({ polygonClosed: false });
  ...
  const openAdd = () => { setForm({ name: '', owner: '', cropTypes: '', soil: '', acreage: '', status: 'Active' }); setErrors({}); setFormCoords([]); setFormBoundaryMeta({ polygonClosed: false }); setShowAddModal(true); };
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.5)' }} onClick={() => setShowAddModal(false)}>
          <div className="w-[560px] max-w-[calc(100vw-32px)] rounded-[24px] p-7 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.3)] border border-white/60" onClick={(e) => e.stopPropagation()}
            ...
            <form onSubmit={handleAdd}>
              <div style={{ background: '#ffffff', borderRadius: '16px', padding: '20px 24px', border: '1px solid rgba(76,175,80,0.15)', marginBottom: '20px', overflow: 'visible' }}>
              ...
              <div style={{ marginTop: '16px', padding: '16px 0 8px', borderTop: '1px solid rgba(0,0,0,0.07)' }}>
                <FarmMapDrawing initialCoords={formCoords} initialClosed={formBoundaryMeta.polygonClosed} onChange={handleFormBoundaryChange} onAcreageChange={handleFormAcreage} modalOpen={showAddModal} />
      ...
      {editFarm && createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.5)' }} onClick={() => setEditFarm(null)}>
          <div className="w-[560px] max-w-[calc(100vw-32px)] rounded-[24px] p-7 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.3)] border border-white/60" onClick={(e) => e.stopPropagation()}
            style={{ background: '#ffffff', maxHeight: 'calc(100vh - 40px)', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <form onSubmit={handleUpdateFarm}>
              ...
              <div style={{ marginTop: '16px', padding: '16px 0 8px', borderTop: '1px solid rgba(0,0,0,0.07)' }}>
                <FarmMapDrawing initialCoords={editCoords} initialClosed={editBoundaryMeta.polygonClosed} onChange={handleEditBoundaryChange} onAcreageChange={handleEditAcreage} modalOpen={!!editFarm} />
      ...
      {deleteFarm && createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setDeleteFarm(null)}>
