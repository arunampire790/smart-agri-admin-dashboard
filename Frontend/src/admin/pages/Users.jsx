...
export default function Users() {
  ...
  const openAdd = () => { setForm({ name: '', email: '', phone: '' }); setErrors({}); setShowAddModal(true); };
  const openView = (user) => setViewUser(user);
  const openEdit = (user) => { setForm({ name: user.name, email: user.email, phone: user.phone }); setErrors({}); setEditUser(user); };
  ...
  const handleAdd = (e) => {
    e.preventDefault();
    if (!validate()) return;
    addUser({ name: form.name.trim(), email: form.email.trim(), phone: form.phone.trim(), farms: 0, status: 'Active', cls: 'bg-brand-light text-brand-dark', joined: new Date().toISOString().slice(0, 10) });
    ...
    <>
      ...
      <div className="glass-card rounded-[20px] p-5 shadow-[0_8px_32px_0_rgba(0,0,0,0.04)]">
        <div className="flex flex-col items-stretch mb-4">
          ...
          <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search users by name or email…" aria-label="Search users" className={inputClass} />
        </div>
            ...
            <tbody>
              {filteredUsers.map((u, i) => (
                <tr key={i}>
                  ...
                  <td className="px-4 py-4 border-b border-table-sep"><span className={`pill inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-semibold ${u.cls}`}>{u.status}</span></td>
                  ...
                  <td className="px-4 py-4 border-b border-table-sep">
                    <div className="flex gap-3 items-center">
                      <button title="View" onClick={() => openView(u)} className="bg-none border-none cursor-pointer text-text-placeholder hover:text-text-secondary text-lg"><i className="ph ph-eye" /></button>
                      <button title="Edit" onClick={() => openEdit(u)} className="bg-none border-none cursor-pointer text-text-placeholder hover:text-text-secondary text-lg"><i className="ph ph-pencil" /></button>
                      <button title="Delete" onClick={() => openDelete(u)} className="bg-none border-none cursor-pointer text-text-placeholder hover:text-danger-text text-lg"><i className="ph ph-trash" /></button>
                    </div>
                  </td>
                </tr>
              ))}
